...
@objc class EstimoteSwift: UIViewController, EILIndoorLocationManagerDelegate {
...
//        )
    ...
    func indoorLocationManager(manager: EILIndoorLocationManager, didUpdatePosition position: EILOrientedPoint, withAccuracy positionAccuracy: EILPositionAccuracy, inLocation location: EILLocation) {
        ...
        let estimotePosition = "\(position.x),\(position.y),\(position.orientation)";
        
        UnitySendMessage("Player", "UpdatePosition", estimotePosition);
//        NSLog(estimotePosition);
